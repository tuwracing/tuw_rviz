...
WeightedPoseWithCovarianceArrayDisplay::WeightedPoseWithCovarianceArrayDisplay()
{
  property_scale_pose_ =
      new rviz::FloatProperty("Scale Pose", 0.4, "Scale of the pose's pose.", this, SLOT(updateScalePose()));
  property_scale_pose_->setMin(0);
  property_scale_pose_->setMax(1);

  property_color_pose_low_ =
      new rviz::ColorProperty("Color Pose Low", QColor(0, 51, 204),
                              "Color to draw the pose's pose at the lowest weight.", this, SLOT(updateColorPose()));

  property_color_pose_high_ =
      new rviz::ColorProperty("Color Pose High", QColor(204, 51, 0),
                              "Color to draw the pose's pose at the highest weight.", this, SLOT(updateColorPose()));

  property_color_variance_ = new rviz::ColorProperty(
      "Color Variance", QColor(204, 51, 204), "Color to draw the pose's variance.", this, SLOT(updateColorVariance()));
}
...
{
  Ogre::ColourValue high = property_color_pose_high_->getOgreColor();
  Ogre::ColourValue low = property_color_pose_low_->getOgreColor();
  visual_->setColorPose(low, high);
}
...
void WeightedPoseWithCovarianceArrayDisplay::processMessage(
    const tuw_geometry_msgs::WeightedPoseWithCovarianceArray::ConstPtr &msg)
{
  ...
  Ogre::ColourValue highColor = property_color_pose_high_->getOgreColor();
  Ogre::ColourValue lowColor = property_color_pose_low_->getOgreColor();
  visual_->setColorPose(highColor, lowColor);
